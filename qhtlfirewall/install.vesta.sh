...
if [ ! -e "/etc/qhtlfirewall/qhtlfirewall.conf" ]; then
	cp -avf qhtlfirewall.vesta.conf /etc/qhtlfirewall/qhtlfirewall.conf
fi
...
if [ ! -e "/etc/qhtlfirewall/qhtlfirewall.allow" ]; then
	cp -avf qhtlfirewall.vesta.allow /etc/qhtlfirewall/qhtlfirewall.allow
fi
...
if [ ! -e "/etc/qhtlfirewall/qhtlfirewall.ignore" ]; then
	cp -avf qhtlfirewall.vesta.ignore /etc/qhtlfirewall/qhtlfirewall.ignore
fi
if [ ! -e "/etc/qhtlfirewall/qhtlfirewall.pignore" ]; then
	cp -avf qhtlfirewall.vesta.pignore /etc/qhtlfirewall/qhtlfirewall.pignore
fi
...
# Ensure qhtlmanager UI is enabled on Vesta installs (safe sed)
...
mkdir -v -m 0600 /usr/local/vesta/web/list/qhtlfirewall/
cp -avf vestacp/* /usr/local/vesta/web/list/qhtlfirewall/
cp -avf qhtlfirewall /usr/local/vesta/web/list/qhtlfirewall/images/
find /usr/local/vesta/web/list/qhtlfirewall -type d -exec chmod -v 755 {} \;
find /usr/local/vesta/web/list/qhtlfirewall -type f -exec chmod -v 644 {} \;
mv /usr/local/vesta/web/list/qhtlfirewall/qhtlfirewall.pl /usr/local/vesta/bin/
