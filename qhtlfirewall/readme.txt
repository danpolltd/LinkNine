...
2. qhtlfirewall Principles

3. qhtlwaterfall Principles

4. qhtlfirewall Command Line Options

5. qhtlwaterfall Command Line Options
...
qhtlfirewall can be used on any (supported - see the website) generic Linux OS.

The qhtlfirewall installation includes preconfigured configurations and control panel
UI's for cPanel, DirectAdmin and Webmin

Directory structure:

/etc/qhtlfirewall/           - configuration files 
/var/lib/qhtlfirewall/       - temporary data files
/usr/local/qhtlfirewall/bin/ - scripts
/usr/local/qhtlfirewall/lib/ - perl modules and static data
/usr/local/qhtlfirewall/tpl/ - email alert templates
...
There are an array of extensive checks that qhtlwaterfall can perform to help alert the
server administrator of changes to the server, potential problems and possible
compromises.

On cPanel servers, qhtlwaterfall is integrated into the WHM > Service Manager, which will
restart qhtlwaterfall if it fails for any reason.

Control Panel Interface
=======================

To help with the ease and flexibility of the suite we have developed a
front-end to both qhtlfirewall and qhtlwaterfall for cPanel, DirectAdmin and Webmin. From there
...
There is, of course, a comprehensive Command Line Interface (CLI) for qhtlfirewall.


2. qhtlfirewall Principles
#################

The idea with qhtlfirewall, as with most iptables firewall configurations, is to block 
...
qhtlfirewall has been designed to keep this configuration simple, but still flexible 
...
qhtlwaterfall can monitor the most commonly abused protocols, SSHD, POP3, IMAP, FTP and 
HTTP password protection. Unlike other applications, qhtlwaterfall is a daemon process 
...
Once the number of failed login attempts is reached, qhtlwaterfall immediately forks a
sub-process and uses qhtlfirewall to block the offending IP address from both in and
...
the attack could be over, or simply biding its time. In the meantime qhtlwaterfall will
...
If you want to know when qhtlwaterfall blocks an IP address you can enable the email
...
Before configuring and starting qhtlfirewall for the first time, it is a good idea to
run the script /etc/qhtlfirewall/qhtlfirewalltest.pl using:

perl /etc/qhtlfirewall/qhtlfirewalltest.pl
...
You can view the qhtlfirewall command line options by using either:
...
These options allow you to easily and quickly control and view qhtlfirewall. All the 
configuration files for qhtlfirewall are in /etc/qhtlfirewall and include:

qhtlfirewall.conf	- the main configuration file, it has helpful comments explaining
		  what each option does
qhtlfirewall.allow	- a list of IP's and CIDR addresses that should always be allowed 
		  through the firewall
qhtlfirewall.deny	- a list of IP's and CIDR addresses that should never be allowed
		  through the firewall
qhtlfirewall.ignore	- a list of IP's and CIDR addresses that qhtlwaterfall should ignore and
		  not block if detected
qhtlfirewall.*ignore	- various ignore files that list files, users, IP's that qhtlwaterfall
		  should ignore. See each file for their specific purpose and
		 tax

If you modify any of the files listed above, you will need to restart qhtlfirewall and
then qhtlwaterfall to have them take effect. If you use the command line options to add
or deny IP addresses, then qhtlfirewall automatically does this for you.

Both qhtlfirewall.allow and qhtlfirewall.deny can have comments after the IP address listed. The
comments must be on the same line as the IP address otherwise the IP rotation
of qhtlfirewall.deny will remove them.

If editing the qhtlfirewall.allow or qhtlfirewall.deny files directly, either from shell or the
WHM UI, you should put a <space>#<space> between the IP address and the comment
like this:

11.22.33.44 # Added because I don't like them

You can also include comments when using the qhtlfirewall -a or qhtlfirewall -d commands, but in
those cases you must not use a # like this:

qhtlfirewall -d 11.22.33.44 Added because I don't like them

If you use the shell commands then each comment line will be timestamped. You
will also find that if qhtlwaterfall blocks an IP address it will add a descriptive
comment plus timestamp.

If you don't want qhtlfirewall to rotate a particular IP in qhtlfirewall.deny if the line limit
...
/etc/qhtlfirewall/qhtlfirewall.allow
/etc/qhtlfirewall/qhtlfirewall.blocklists
/etc/qhtlfirewall/qhtlfirewall.cloudflare
/etc/qhtlfirewall/qhtlfirewall.deny
/etc/qhtlfirewall/qhtlfirewall.dirwatch
/etc/qhtlfirewall/qhtlfirewall.dyndns
/etc/qhtlfirewall/qhtlfirewall.fignore
/etc/qhtlfirewall/qhtlfirewall.ignore
/etc/qhtlfirewall/qhtlfirewall.logfiles
/etc/qhtlfirewall/qhtlfirewall.logignore
/etc/qhtlfirewall/qhtlfirewall.mignore
/etc/qhtlfirewall/qhtlfirewall.pignore
/etc/qhtlfirewall/qhtlfirewall.rblconf
/etc/qhtlfirewall/qhtlfirewall.redirect
/etc/qhtlfirewall/qhtlfirewall.rignore
/etc/qhtlfirewall/qhtlfirewall.signore
/etc/qhtlfirewall/qhtlfirewall.sips
/etc/qhtlfirewall/qhtlfirewall.smtpauth
/etc/qhtlfirewall/qhtlfirewall.suignore
/etc/qhtlfirewall/qhtlfirewall.syslogs
/etc/qhtlfirewall/qhtlfirewall.syslogusers
/etc/qhtlfirewall/qhtlfirewall.uidignore

You must specify the full path to the included file, e.g. in
/etc/qhtlfirewall/qhtlfirewall.allow:

Include /etc/qhtlfirewall/qhtlfirewall.alsoallow

Do NOT put a comment after the Include filename as this will not work and will
invalidate the Include line.

Note: None of the qhtlfirewall commands for adding or removing entries from the
...
qhtlwaterfall does not have any command line options of its own but is controlled through
init or systemd which stops and starts the daemon. It is configured using the
/etc/qhtlfirewall/qhtlfirewall.conf file.

The best way to see what qhtlwaterfall is up to is to take a look in /var/log/qhtlwaterfall.log 
...
/usr/local/qhtlfirewall/tpl/accounttracking.txt - for account tracking alert emails
/usr/local/qhtlfirewall/tpl/alert.txt - for port blocking emails
/usr/local/qhtlfirewall/tpl/connectiontracking.txt - for connection tracking emails
/usr/local/qhtlfirewall/tpl/consolealert.txt - for console root login alert emails
/usr/local/qhtlfirewall/tpl/cpanelalert.txt - for WHM/cPanel account access emails
/usr/local/qhtlfirewall/tpl/exploitalert.txt - for system exploit alert emails
/usr/local/qhtlfirewall/tpl/filealert.txt - for suspicious file alert emails
/usr/local/qhtlfirewall/tpl/forkbombalert.txt - for fork bomb alert emails
/usr/local/qhtlfirewall/tpl/integrityalert.txt - for system integrity alert emails
/usr/local/qhtlfirewall/tpl/loadalert.txt - for high load average alert emails
/usr/local/qhtlfirewall/tpl/logalert.txt - for log scanner report emails
/usr/local/qhtlfirewall/tpl/logfloodalert.txt - for log file flooding alert emails
/usr/local/qhtlfirewall/tpl/modsecipdbcheck.txt - for ModSecurity IP DB size alert emails
/usr/local/qhtlfirewall/tpl/netblock.txt - for netblock alert emails
/usr/local/qhtlfirewall/tpl/permblock.txt - for temporary to permanent block alert emails
/usr/local/qhtlfirewall/tpl/portknocking.txt - for Port Knocking alert emails
/usr/local/qhtlfirewall/tpl/portscan.txt - for port scan tracking alert emails
/usr/local/qhtlfirewall/tpl/processtracking.txt - for process tracking alert emails
/usr/local/qhtlfirewall/tpl/queuealert.txt - for email queue alert emails
/usr/local/qhtlfirewall/tpl/relayalert.txt - for email relay alert emails
/usr/local/qhtlfirewall/tpl/resalert.txt - for process resource alert emails
/usr/local/qhtlfirewall/tpl/scriptalert.txt - for script alert emails
/usr/local/qhtlfirewall/tpl/sshalert.txt - for SSH login emails
/usr/local/qhtlfirewall/tpl/sualert.txt - for SU alert emails
/usr/local/qhtlfirewall/tpl/tracking.txt - for POP3/IMAP blocking emails
/usr/local/qhtlfirewall/tpl/uialert.txt - for UI alert emails
/usr/local/qhtlfirewall/tpl/usertracking.txt - for user process tracking alert emails
/usr/local/qhtlfirewall/tpl/watchalert.txt - for watched file and directory change alert emails
/usr/local/qhtlfirewall/tpl/webminalert.txt - for Webmin login emails

6. Login Tracking
#################

Login tracking is an extension of qhtlwaterfall, it keeps track of POP3 and IMAP logins
...
temporary and can be cleared manually by restarting qhtlfirewall.
...
If you want to know when qhtlwaterfall temporarily blocks an IP address you can enable
...
matching. Please read /usr/local/qhtlfirewall/bin/regex.custom.pm for more information
...
(cPanel installations of qhtlfirewall only)

qhtlwaterfall can scan for emails being sent through exim from scripts on the server.
...
This option uses the /usr/local/qhtlfirewall/tpl/scriptalert text file for alert emails.

If you enable the option LF_SCRIPT_ALERT then qhtlwaterfall will disable the path using
...
Any false-positives can be added to /etc/qhtlfirewall/qhtlfirewall.signore and qhtlwaterfall will then
...
reported once unless qhtlwaterfall is restarted.

There is an ignore file /etc/qhtlfirewall/qhtlfirewall.pignore which can be used to whitelist
...
positives before either ignoring them or trapping them in the qhtlfirewall.pignore file.
...
Directory Watching enables qhtlwaterfall to check /tmp and /dev/shm and other pertinent
...
NOTE: Only one alert per file is sent until qhtlwaterfall is restarted, so if you remove
a suspicious file, remember to restart qhtlwaterfall
...
/var/lib/qhtlfirewall/suspicious.tar and deleted from their original location. Symlinks
are simply removed.

If you want to extract the tarball to your current location, use:

tar -xpf /var/lib/qhtlfirewall/suspicious.tar

This will preserver the path and permissions of the original file.

Any false-positives can be added to /etc/qhtlfirewall/qhtlfirewall.fignore and qhtlwaterfall will then
ignore those listed files and directories.

Within qhtlfirewall.fignore is a list of files that qhtlwaterfall directory watching will ignore.
...
option allows you to have qhtlwaterfall watch a particular file or directory for changes
...
In /etc/qhtlfirewall/qhtlfirewall.allow and /etc/qhtlfirewall/qhtlfirewall.deny you can add more complex port and
...
qhtlfirewall+qhtlwaterfall can be configured to run on a generic Linux server. There are some
...
2. The Process Tracking ignore file may need expanding in /etc/qhtlfirewall/qhtlfirewall.pignore
   to suit the server environment

3. A standard Webmin Module to configure qhtlfirewall is included - see the install.txt
   for more information

The codebase is the same for a all installations, the qhtlfirewall.conf file simply has
...
in both qhtlfirewall and your FTP server configuration (see above).
...
Temporary and/or permanent (qhtlfirewall.deny) IP addresses can be serviced by this
feature.
...
If you change any of the files in /etc/qhtlfirewall/messenger/ you must restart qhtlwaterfall as
they are all cached in memory.

Use of this feature can be controlled by the Country Code options:
CC_MESSENGER_ALLOW = ""
CC_MESSENGER_DENY = ""
See /etc/qhtlfirewall/qhtlfirewall.conf for an explanation of those options.
...
For example, you can create such an account (in this example called "qhtlfirewall") from
the root shell using:

useradd qhtlfirewall -s /bin/false
...
/etc/qhtlfirewall/messenger/index.text
...
/etc/qhtlfirewall/messenger/index.html
/etc/qhtlfirewall/messenger/index.recaptcha.html (if using the RECAPTCHA_* feature)

The HTML server providing this page is very rudimentary but will accept the use
of linked images that are stored in the /etc/qhtlfirewall/messenger/ directory. The
...
load speed and reduce qhtlwaterfall overheads.
...
The web server configuration is created in /var/lib/qhtlfirewall/qhtlfirewall.conf using the
following templates in /usr/local/qhtlfirewall/tpl/:
...
These templates are not overwritten during a qhtlfirewall upgrade.
...
not work, this should be set as an "Include /path/to/qhtlfirewall_php.conf" or similar
...
[MESSENGERV3PHPHANDLER] line from the qhtlfirewall webserver template files.
...
the standard web server setup, e.g. using "qhtlfirewall" as the user:

/home/qhtlfirewall/ (Owner qhtlfirewall:qhtlfirewall, Permissions 711)
/home/qhtlfirewall/public_html/ (Owner qhtlfirewall:nobody, Permissions 711)

qhtlwaterfall will populate this structure with the following files:

/home/qhtlfirewall/public_html/.htaccess
/home/qhtlfirewall/public_html/index.php

If RECAPTCHA_* is enabled these files will be created if they do not already
exist:

/home/qhtlfirewall/recaptcha.php
/home/qhtlfirewall/public_html/index.php
/home/qhtlfirewall/en.php

The HTML and HTTPS index file is created from (respectively):
/etc/qhtlfirewall/messenger/index.php
/etc/qhtlfirewall/messenger/index.recaptcha.php
/etc/qhtlfirewall/messenger/en.php

You should NOT modify the templates in /etc/qhtlfirewall/messenger/ as they will be
overwritten when qhtlfirewall upgrades. Instead modify the files within /home/qhtlfirewall/.

Each time qhtlwaterfall is restarted a check is made of the preceding structure and any
missing files are recreated. This process also creates the configuration file
for Apache in /etc/apache2/conf.d/qhtlfirewall.messenger.conf and restarts httpd.

/etc/apache2/conf.d/qhtlfirewall.messenger.conf contains all the VirtualHost directives
to serve the MESSENGERV2 services.

Translation of /home/qhtlfirewall/en.php is possible by creating the appropriate
[abbr].php file.

The HTML and HTTPS server ports should NOT be added to the TCP_IN list.

As Apache is handling all requests for HTML and HTTPS connections, all
scripting for the service is provided by the files in /home/qhtlfirewall/public_html/
...
qhtlwaterfall can run an external script when it performs and IP address block following
...
qhtlwaterfall launches the BLOCK_REPORT in a forked process which terminates after 10
...
To also run an external script when a temporary block is unblocked by qhtlwaterfall.
...
Restarting qhtlfirewall resets the ipt_recent tables and removes all of its blocks.
...
External commands (e.g. iptables rules not covered by qhtlfirewall) can be run before
and/or after qhtlfirewall sets up the iptables chains and rules.

1. To run external commands before qhtlfirewall configures iptables create the file:

/usr/local/qhtlfirewall/bin/qhtlfirewallpre.sh
...
Then chmod +x /usr/local/qhtlfirewall/bin/qhtlfirewallpre.sh

2. To run external commands after qhtlfirewall configures iptables create the file:

/usr/local/qhtlfirewall/bin/qhtlfirewallpost.sh

Set that file as executable and add an appropriate shebang interpreter line and
then whatever external commands you wish to execute.


Note: The scripts can alternatively be placed in /etc/qhtlfirewall/. If a script is found in
both locations (/etc/qhtlfirewall/ and /usr/local/qhtlfirewall/bin/) then only the script in
/usr/local/qhtlfirewall/bin/ will be executed.

qhtlfirewallpre.sh/qhtlfirewallpost.sh are run directly. If present, qhtlfirewall chmods the script 0700
and checks for a shebang. If the shebang is missing #!/bin/bash is added to the
top. The script is them run.

Note: While qhtlfirewall runs the script with a preset PATH, you MUST use the full path
...
This set of options (CLUSTER*) in qhtlfirewall.conf allows the configuration of an
qhtlwaterfall cluster environment where a group of servers can share blocks and, via the
...
If you want all members of the qhtlwaterfall cluster to send block notifications to each
other then both settings should be them same. You also need to enable
CLUSTER_BLOCK (enabled by default) for qhtlwaterfall to automatically send blocks to all
...
NOT be opened in TCP_IN or TCP_OUT as qhtlfirewall will automatically add appropriate in
...
Note: You must restart qhtlfirewall and then qhtlwaterfall after making any CLUSTER_* changes

qhtlwaterfall Cluster CLI and UI
======================

See qhtlfirewall --help for the list of new CLI commands. Additional options will
...
The CLI options --cfile and --cfiler allow you to synchronise qhtlfirewall configuration
...
the request in their respective qhtlwaterfall.log files. This is intended as a test to
confirm that cluster communications are functioning.

The options to change the configuration option in qhtlfirewall.conf in cluster members
...
restart qhtlfirewall and qhtlwaterfall to effect the requested changes immediately.


A Note on qhtlwaterfall Cluster Security
==============================
...
1. qhtlfirewall constructs iptables rules such that only cluster members can communicate
...
flaw found in the encryption modules or their implementation in qhtlfirewall, a
malicious connection could reconfigure the qhtlfirewall firewall and then leverage a
...
Restarting qhtlfirewall resets the ipt_recent tables and removes all of the knocks.
...
Note: Run /etc/qhtlfirewall/qhtlfirewalltest.pl to check whether this option will function on the
server
...
This feature uses the file /etc/qhtlfirewall/qhtlfirewall.redirect which is a list of port and/or
...
work. qhtlfirewall will set this where it can, but if the kernel value cannot be set
...
qhtlfirewall and qhtlwaterfall, without requiring a control panel or web server. The UI runs as a
sub process to the qhtlwaterfall daemon.
...
Requirements:
  ...
  4. SSL keys
  5. Entries in /etc/qhtlfirewall/ui/ui.allow

The SSL server uses the following files:

  SSL Key goes into /etc/qhtlfirewall/ui/server.key
  SSL Certificate goes into /etc/qhtlfirewall/ui/server.crt
...
file. qhtlwaterfall must be restarted after making any changes:
...
UI must be listed in /etc/qhtlfirewall/ui/ui.allow before trying to connect to the UI.

Only IP addresses can be listed/used in /etc/qhtlfirewall/ui/ui.ban - this file should
only be used by the UI to prevent login. Use qhtlfirewall blocks to prevent access to
the configured port and only use Advanced Allow/Deny Filters for access, i.e.
do not list the port in TCP_IN.

Logging for UI events are logged to the qhtlwaterfall /var/log/qhtlwaterfall.log file. Check this
...
This feature allows qhtlfirewall/qhtlwaterfall to periodically download lists of IP addresses and
CIDRs from pubished block or black lists. It is controlled by the file:
/etc/qhtlfirewall/qhtlfirewall.blocklists

Uncomment the line starting with the rule name to use it, then restart qhtlfirewall and
then qhtlwaterfall.
...
After making any changes to this file you must restart qhtlfirewall and then qhtlwaterfall.

If you want to redownload a blocklist you must first delete
/var/lib/qhtlfirewall/qhtlfirewall.block.NAME and then restart qhtlfirewall and then qhtlwaterfall.
...
monitor these logs, such as qhtlwaterfall does for the following options:
...
/etc/qhtlfirewall/qhtlfirewall.allow and perhaps /etc/qhtlfirewall/qhtlfirewall.ignore. This will prevent malicious
...
other than qhtlfirewall/qhtlwaterfall that use the affected log files
...
IP addresses listed in /etc/qhtlfirewall/qhtlfirewall.smtpauth plus the localhost IP addresses.
...
Note: qhtlfirewall and qhtlwaterfall must be restarted if /etc/qhtlfirewall/qhtlfirewall.smtpauth is modified so
that the lookup file in /etc/exim.smtpauth is regenerated from the information
from /etc/qhtlfirewall/qhtlfirewall.smtpauth, the localhost IP addresses, plus any countries
...
7. That should be all that is required after having made any necessary changes
   within qhtlfirewall.conf and restarting qhtlfirewall and then qhtlwaterfall
...
5. Disable SMTPAUTH_RESTRICT and CC_ALLOW_SMTPAUTH in qhtlfirewall.conf and then restart
   qhtlfirewall and then qhtlwaterfall
...
The qhtlfirewall UI provided through cPanel, DirectAdmin, Webmin and the integrated UI
via qhtlwaterfall, all user the Bootstrap and jQuery frameworks. Additional styling is
...
1. A text file /etc/qhtlfirewall/qhtlfirewall.header which will be included in each of
the UI pages before the closing </head> tag

2. A text file /etc/qhtlfirewall/qhtlfirewall.body which will be included in each of the UI
pages after the opening <body> tag[*]

3. A text file /etc/qhtlfirewall/qhtlfirewall.footer which will be included in each of the UI
...
/etc/qhtlfirewall/qhtlfirewall.htmltag and /etc/qhtlfirewall/qhtlfirewall.bodytag respectively[*]. Additions made
...
single line in /etc/qhtlfirewall/qhtlfirewall.bodytag:
data-name='result'
...
There are options in qhtlfirewall.conf that control the behaviour of these options under
...
3.  The temporary/permanent config of the qhtlwaterfall settings are ignored and CF_TEMP
    is used instead
...
9.  When restarting qhtlfirewall, any old temporary blocks will still be created for qhtlwaterfall
...
NOTE: You should always list the CloudFlare IP addresses in /etc/qhtlfirewall/qhtlfirewall.ignore
to prevent them from being blocked by qhtlwaterfall from https://www.cloudflare.com/ips/


CLI commands
============

There are also accompanying qhtlfirewall CLI commands available (see man) to interact
...
qhtlfirewall --cloudflare list all [user1,user2,...]

2. Block, challenge and whitelist rules can be added for IPs to the provided
users CloudFlare firewall, e.g.:

qhtlfirewall --cloudflare add challenge 11.22.33.44 [user1,user2,...]

Note: These rules are NOT cleared by qhtlwaterfall and do NOT create an equivalent
iptables rule in qhtlfirewall)

3. Rules can be deleted for IPs to the provided users CloudFlare firewall,
e.g.:

qhtlfirewall --cloudflare del 11.22.33.44 [domain,domain2,...]

Note: These rules are NOT cleared in qhtlfirewall if they exist

4. Domains can also be used instead of users, or a mixture of both e.g.:

qhtlfirewall --cloudflare list all [user,user2,domain,...]

5. IPs can be added both the users CloudFlare firewall and to qhtlfirewall as temporary
allow or deny, e.g.:

qhtlfirewall --cloudflare tempadd deny 11.22.33.44 [user1,user2,...]

Note: This applies the allow/deny for the IP address in qhtlfirewall for CF_TEMP seconds
as well as the users CloudFlare Firewall. Once the temporary entry expires qhtlwaterfall
removes the IP address from both qhtlfirewall (as normal) as well as the users
CloudFlare Firewall.

Note: Any CloudFlare account configured to use the special case "any" field
value in qhtlfirewall.cloudflare will also have a CloudFlare rule added.
...
6. To manually remove an IP block that was blocked via CF_ENABLE in qhtlwaterfall or by
using "tempadd" use the normal qhtlfirewall temp CLI commands. This will remove the
rules from both iptables and the users CloudFlare firewall, e.g.:

qhtlfirewall --tr 44.33.22.11


28. InterWorx
#############

InterWorx integration is available for qhtlfirewall. The installation makes changes to
the underlying InterWorx installation due to its current dependence on APF. To
cater for this, installing qhtlfirewall will replace /etc/apf/apf with a stub script
that redirects commands to qhtlfirewall. The script is then chattr +ia to prevent it
being overwritten.

Note: None of the apf conf files are used and are ignored by qhtlfirewall.
...
NOTE: Unless you have configured a root forwarder, you should edit the qhtlfirewall
configuration settings in /etc/qhtlfirewall/qhtlfirewall.conf or via the UI and set LF_ALERT_TO
to a suitable email address. After making any changes, restart qhtlfirewall and then
qhtlwaterfall.
...
there is a custom entry in /etc/qhtlfirewall/regex.custom.pm. The now official detection
will be ignored while this is in place.

If you want to use the now inbuilt detection you must edit
/etc/qhtlfirewall/regex.custom.pm and remove the 3 lines that comprise the custom entry
and then restart qhtlwaterfall.
