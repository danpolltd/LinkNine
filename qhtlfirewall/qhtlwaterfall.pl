...
our ($abuseip, $accept, $apache401timeout, $apache403timeout,
	 ...
	 $clusterip, $count, $qhtlfirewalltimeout, $cttimeout, $cxsreputation,
...
logfile("daemon started on $hostname - qhtlfirewall v$version$generic");
...
if (QhtLink::Service::type() eq "systemd") {
	...
	if ($reply[0] eq "active" or $reply[0] eq "activating") {
		&cleanup(__LINE__,"*Error* firewalld found to be running. You must stop and disable firewalld when using qhtlfirewall");
		exit 1;
	}
}
...
if ($config{UI}) {
	...
	if ($config{UI_USER} eq "" or $config{UI_USER} eq "username") {
		...
		logfile("*Error* Cannot run Integrated qhtlfirewall UI - UI_PASS must set");
		$config{UI} = 0;
	}
	else {
		logfile("qhtlfirewall Integrated UI running up on port $config{UI_PORT}...");
		&ui;
	}
}
...
if ($config{DYNDNS}) {
	...
	$dyndnstimeout = 0;
	if ($config{DYNDNS} < 60) {
		logfile("DYNDNS refresh increased to 300 to prevent looping (qhtlfirewall.conf setting: $config{DYNDNS})");
		$config{DYNDNS} = 300;
	}
}
...
if ($config{LF_GLOBAL}) {
	...
	if ($config{LF_GLOBAL} < 60) {
		logfile("LF_GLOBAL refresh increased to 300 to prevent looping (qhtlfirewall.conf setting: $config{LF_GLOBAL})");
		$config{LF_GLOBAL} = 300;
	}
	if ($config{GLOBAL_DYNDNS_INTERVAL} < 60) {
		logfile("GLOBAL_DYNDNS_INTERVAL refresh increased to 300 to prevent looping (qhtlfirewall.conf setting: $config{GLOBAL_DYNDNS_INTERVAL})");
...
if ($config{LF_INTEGRITY}) {
	...
	if ($config{LF_INTEGRITY} < 120) {
		logfile("LF_INTEGRITY refresh increased to 300 to prevent looping (qhtlfirewall.conf setting: $config{LF_INTEGRITY})");
...
if ($config{LF_EXPLOIT}) {
	...
	if ($config{LF_EXPLOIT} < 60) {
		logfile("LF_EXPLOIT refresh increased to 60 to prevent looping (qhtlfirewall.conf setting: $config{LF_EXPLOIT})");
...
if ($config{LF_QUEUE_ALERT}) {
	...
	if ($config{LF_QUEUE_INTERVAL} < 30) {
		logfile("LF_QUEUE_INTERVAL refresh increased to 300 to prevent looping (qhtlfirewall.conf setting: $config{LF_QUEUE_INTERVAL})");
...
if ($config{PS_INTERVAL}) {
	...
	if ($config{PS_INTERVAL} < 60) {
		logfile("PS_INTERVAL refresh increased to 60 to prevent looping (qhtlfirewall.conf setting: $config{PS_INTERVAL})");
...
if ($config{UID_INTERVAL}) {
	...
	if ($config{UID_INTERVAL} < 60) {
		logfile("UID_INTERVAL refresh increased to 60 to prevent looping (qhtlfirewall.conf setting: $config{UID_INTERVAL})");
...
if ($config{CT_LIMIT}) {
	...
	if ($config{CT_INTERVAL} < 10) {
		logfile("CT_INTERVAL refresh increased to 30 to prevent looping (qhtlfirewall.conf setting: $config{CT_INTERVAL})");
...
if ($config{PT_LIMIT}) {
	...
	if ($config{PT_INTERVAL} < 10) {
		logfile("PT_INTERVAL refresh increased to 60 to prevent looping (qhtlfirewall.conf setting: $config{PT_INTERVAL})");
...
if ($config{AT_ALERT}) {
	...
	if ($config{AT_INTERVAL} < 10) {
		logfile("AT_INTERVAL refresh increased to 60 to prevent looping (qhtlfirewall.conf setting: $config{AT_INTERVAL})");
...
if ($config{LF_INTERVAL} < 60) {
	logfile("LF_INTERVAL refresh increased to 300 to prevent looping (qhtlfirewall.conf setting: $config{LF_INTERVAL})");
	$config{LT_INTERVAL} = 300;
}

if ($config{LF_PARSE} < 5 or $config{LF_PARSE} > 20) {
	logfile("LF_PARSE refresh reset to 5 to prevent looping (qhtlfirewall.conf setting: $config{LF_PARSE})");
	$config{LF_PARSE} = 5;
}
	...
	my ($app, $account, $ip) = QhtLink::RegexMain::processloginline ($line);
		...
		if ($account and $loginproto{$app} and !&ignoreip($ip,1)) {
			...
			if ($logins{$app}{$account}{$ip} > $loginproto{$app}) {
				$0 = "qhtlwaterfall - disabling $app logins for $account";
...
sub lockhang {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) checking for lock hang";
	...
	unless ($childpid) {
		...
		if ($blocked) {
			if ($config{LF_EMAIL_ALERT} and ($perm or (!$perm and $config{LF_TEMP_EMAIL_ALERT}))) {
				$0 = "qhtlwaterfall - (child) sending alert email for $ip";
				...
				if ($allowip == 1) {$block .= " (IP match in qhtlfirewall.allow, block may not work)"}
			...
			if ($config{X_ARF}) {
				...
				foreach my $line (@alert) {
					...
					$line =~ s/\[qhtlfirewallversion\]/$version/ig;
...
sub blockdistftp {
	...
	unless ($childpid) {
		...
		if ($config{LF_DIST_ACTION} and -e $config{LF_DIST_ACTION} and -x $config{LF_DIST_ACTION}) {
			$0 = "qhtlwaterfall - (child) running LF_DIST_ACTION";
...
sub logindisable {
	...
	unless ($childpid) {
		...
		if (&ipblock(0,"$app - $logins logins in $logintimeout{$app} secs from $tip for $account exceeds $loginproto{$app}/hour",$ip,"$port,$sport","in",$flush,0,"",$trigger)) {
			...
			if ($config{LT_EMAIL_ALERT}) {
				$0 = "qhtlwaterfall - (child) sending alert email for $account";
...
sub uidscans {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) sending alert email for UID $uid";
...
# end uidscans
###############################################################################
# start qhtlfirewallrestart
sub qhtlfirewallrestart {
	my $timer = time;
	if ($config{DEBUG} >= 3) {$timer = &timer("start","qhtlfirewallrestart",$timer)}
	$0 = "qhtlwaterfall - (re)starting qhtlfirewall...";

	logfile("qhtlfirewall (re)start requested - running *qhtlfirewall startup*...");
	&syscommand(__LINE__,"/usr/sbin/qhtlfirewall","-sf");
	logfile("qhtlfirewall (re)start completed");

	if ($config{DEBUG} >= 3) {$timer = &timer("stop","qhtlfirewallrestart",$timer)}
	$0 = "qhtlwaterfall - processing";
	return;
}
# end qhtlfirewallrestart
###############################################################################
# start qhtlwaterfallrestart
sub qhtlwaterfallrestart {
	$SIG{INT} = 'IGNORE';
	$SIG{TERM} = 'IGNORE';
	$SIG{CHLD} = 'IGNORE';
	$0 = "qhtlwaterfall - stopping";
	...
	$SIG{HUP} = 'IGNORE';
	kill HUP => -$$;
	exec("/usr/sbin/qhtlwaterfall");

	exit 0;
}
# end qhtlwaterfallrestart
###############################################################################
# start qhtlfirewallcheck
sub qhtlfirewallcheck {
	my $timer = time;
	if ($config{DEBUG} >= 3) {$timer = &timer("start","qhtlfirewallcheck",$timer)}
	...
	if ($ipdata[0] =~ /xtables lock/) {
		logfile("*Error*: Unable to check qhtlfirewall due to xtables lock, enable WAITLOCK in qhtlfirewall.conf");
	} else {
		if ($ipdata[0] !~ /^Chain LOCALINPUT/) {
			$0 = "qhtlwaterfall - starting qhtlfirewall...";
			logfile("iptables appears to have been flushed - running *qhtlfirewall startup*...");
			&syscommand(__LINE__,"/usr/sbin/qhtlfirewall","-sf");
			logfile("qhtlfirewall startup completed");
			$0 = "qhtlwaterfall - processing";
		}
	...
	if ($config{DEBUG} >= 3) {$timer = &timer("stop","qhtlfirewallcheck",$timer)}
	return;
}
# end qhtlfirewallcheck
###############################################################################
# start loadcheck
sub loadcheck {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) checking load...";
...
sub queuecheck {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) checking mail queue...";
...
sub modsecipdbcheck {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) checking modsec ip db...";
...
sub connectiontracking {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) connection tracking...";
			...
			if (($ipcnt{$ip} > $config{CT_LIMIT}) and !&ignoreip($ip)) {
				...
				if (&ipblock($config{CT_PERMANENT},"(CT) IP $tip found to have $ipcnt{$ip} connections",$ip,"","inout",$config{CT_BLOCK_TIME},0,$iptext{$ip},"CT_LIMIT")) {
					...
					if ($config{CT_EMAIL_ALERT}) {
						$0 = "qhtlwaterfall - (child) (CT) sending alert email for $ip";
		...
		foreach my $subnet (keys %subcnt) {
			...
			if (($subcnt{$subnet} > $config{CT_SUBNET_LIMIT}) and !&ignoreip($fullsubnet)) {
				...
				if (&ipblock($config{CT_PERMANENT},"(CT) subnet $tip found to have $subcnt{$subnet} connections",$fullsubnet,"","inout",$config{CT_BLOCK_TIME},0,$subtext{$subnet},"CT_SUBNET_LIMIT")) {
					...
					if ($config{CT_EMAIL_ALERT}) {
						$0 = "qhtlwaterfall - (child) (CT) sending alert email for $subnet";
						...
						if ($allowip == 1) {$block .= " (subnet match in qhtlfirewall.allow, block may not work)"}
...
sub accounttracking {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) account tracking...";
...
sub syslogcheck {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) SYSLOG check...";
...
sub processtracking {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) process tracking...";
		...
		while (my $pid = readdir(PROCDIR)) {
			...
			if ($users{$user} or $config{GENERIC} or $config{PT_ALL_USERS}) {
				...
				if (($config{MESSENGER} and $user eq $config{MESSENGER_USER}) and ($cmdline =~ /^qhtlwaterfall (HTML|TEXT|HTTPS) messenger/)) {next}
				...
				if ($uptime > $config{PT_LIMIT}) {
					...
					if ($suspect or $deleted) {
						...
						$0 = "qhtlwaterfall - (child) (PT) sending alert email for process $pid";
		...
		if ($config{PT_USERPROC}) {
			$0 = "qhtlwaterfall - (child) (PT) checking user processes";
...
sub sshalert {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) sending SSH login alert email for $ip";
...
sub sualert {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) sending SU login alert email from $sufrom to $suto";
...
sub webminalert {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) sending Webmin login alert email for $ip";
...
sub consolealert {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) sending console login alert email";
...
sub cpanelalert {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) sending WHM/cPanel access alert email for $ip";
...
sub scriptalert {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) identifying possible email scripts";
		...
		$0 = "qhtlwaterfall - (child) sending script alert";
...
sub relayalert {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) reporting exceeded $check limit";
		...
		my $allowip = &allowip($ip);
		if ($allowip == 1 and $block ne "No") {$block .= " (IP match in qhtlfirewall.allow, block may not work)"}
...
sub portknocking {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) sending Port Knocking alert email for $ip";
...
sub blocklist {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - retrieving blocklists";
		...
		$0 = "qhtlwaterfall - retrieving blocklists (waiting for list lock)";
		&listlock("lock");
		...
		foreach my $name (keys %blocklists) {
			...
			if ($getlist) {
				$0 = "qhtlwaterfall - retrieving blocklist $name";
				...
				if (&qhtlfirewalllock) {&lockfail("BLOCKLIST")}
...
sub countrycode {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - retrieving countrycode lists";
		...
		$0 = "qhtlwaterfall - retrieving countrycode lists (waiting for list lock)";
		&listlock("lock");
		$0 = "qhtlwaterfall - retrieving countrycode lists";
		...
		if ($config{CC_SRC} eq "" or $config{CC_SRC} eq "1") {
			...
			$0 = "qhtlwaterfall - processing countrycode lists";
			foreach my $cc (split(/\,/,"$config{CC_DENY},$config{CC_ALLOW},$config{CC_ALLOW_FILTER},$config{CC_ALLOW_PORTS},$config{CC_DENY_PORTS},$config{CC_ALLOW_SMTPAUTH}")) {
				if ($cc) {
					$cc = lc $cc;
		...
		if ($config{LF_IPSET}) {
			...
			if ($config{CC_DENY} and $redo_deny) {
				if (&qhtlfirewalllock) {&lockfail("CC_DENY")}
				if ($config{SAFECHAINUPDATE}) {
			...
			if ($config{CC_ALLOW} and $redo_allow) {
				if (&qhtlfirewalllock) {&lockfail("CC_ALLOW")}
				if ($config{SAFECHAINUPDATE}) {
			...
			if ($config{CC_ALLOW_FILTER} and $redo_allow_filter) {
				my $cnt = 0;
				if (&qhtlfirewalllock) {&lockfail("CC_ALLOW_FILTER")}
				if ($config{SAFECHAINUPDATE}) {
			...
			if ($config{CC_ALLOW_PORTS} and $redo_allow_ports) {
				my $cnt = 0;
				if (&qhtlfirewalllock) {&lockfail("CC_ALLOW_PORTS")}
				if ($config{SAFECHAINUPDATE}) {
			...
			if ($config{CC_DENY_PORTS} and $redo_deny_ports) {
				my $cnt = 0;
				if (&qhtlfirewalllock) {&lockfail("CC_DENY_PORTS")}
				if ($config{SAFECHAINUPDATE}) {
...
sub countrycodelookups {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - retrieving countrycode lookups";
		...
		$0 = "qhtlwaterfall - retrieving countrycodelookups lists (waiting for list lock)";
		&listlock("lock");
		$0 = "qhtlwaterfall - retrieving countrycodelookups lists";
...
sub countrycode6 {
	...
	$0 = "qhtlwaterfall - processing countrycode6 lists";
	foreach my $cc (split(/\,/,"$config{CC_DENY},$config{CC_ALLOW},$config{CC_ALLOW_FILTER},$config{CC_ALLOW_PORTS},$config{CC_DENY_PORTS},$config{CC_ALLOW_SMTPAUTH}")) {
		if ($cc eq "") {next}
		$cc = lc $cc;
...
sub global {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - retrieving global lists";
		...
		$0 = "qhtlwaterfall - retrieving global lists (waiting for list lock)";
		&listlock("lock");
		$0 = "qhtlwaterfall - retrieving global lists";
		...
		if ($config{GLOBAL_ALLOW}) {
			...
			if ($status) {
				...
				if (&qhtlfirewalllock) {&lockfail("GLOBAL_ALLOW")}
		...
		if ($config{GLOBAL_DENY}) {
			...
			if ($status) {
				...
				if (&qhtlfirewalllock) {&lockfail("GLOBAL_DENY")}
...
sub dyndns {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - resolving dyndns IP addresses";
		...
		$0 = "qhtlwaterfall - resolving dyndns IP addresses (waiting for list lock)";
		&listlock("lock");
		$0 = "qhtlwaterfall - resolving dyndns IP addresses";
		...
		if (&qhtlfirewalllock) {&lockfail("DYNDNS")}
...
sub globaldyndns {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - resolving global dyndns IP addresses";
		...
		$0 = "qhtlwaterfall - resolving global dyndns IP addresses (waiting for list lock)";
		&listlock("lock");
		$0 = "qhtlwaterfall - resolving global dyndns IP addresses";
		...
		if (&qhtlfirewalllock) {&lockfail("GLOBAL_DYNDNS_INTERVAL")}
...
sub dirwatch {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - checking directories";
		...
		if ($@) {
			...
			if (@suspicious) {
				$0 = "qhtlwaterfall - reporting directory watch results";
				...
				foreach my $file (@suspicious) {
					...
					logfile($line);
					$0 = "qhtlwaterfall - (child) suspicious file alert for $file";
...
sub dirwatchfile {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - checking files and directories";
...
sub integrity {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - checking system integrity";
		...
		if ($action eq "start") {
			...
			if ($@) {
				...
				if ($report) {
					...
					$0 = "qhtlwaterfall - (child) system integrity alert";
...
sub exploit {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - checking system exploit";
		...
		if ($report) {
			$0 = "qhtlwaterfall - (child) system exploit alert";
...
sub iptablescmd {
	...
	if ($status) {
		...
		if (&qhtlfirewalllock) {
			logfile("qhtlfirewall is currently restarting - command [$command] skipped on line $line");
		...
		if ($output[0] =~ /^deny failed\:/) {logfile("*Error*: qhtlfirewall output: $output[0]")}
		if ($output[0] =~ /^Error\:/) {logfile("*Error*: qhtlfirewall output: $output[0]")}
		if ($output[0] =~ /xtables lock/) {logfile("*Error*: Unable to check qhtlfirewall due to xtables lock, enable WAITLOCK in qhtlfirewall.conf")}
...
sub syscommand {
	...
	if ($status) {
		...
		if (&qhtlfirewalllock) {
			logfile("qhtlfirewall is currently restarting - command [$cmdline] skipped on line $line");
			unless ($masterpid == $$) {exit}
		}
		my ($childin, $childout);
...
# end qhtlfirewalllock
###############################################################################
# start lockfail
sub lockfail {
	my $section = shift;
	logfile("qhtlfirewall is currently restarting - section [$section] skipped");
	exit;
}
# end lockfail
###############################################################################
...
#						if ($block eq $oip) {$nipmatch = 1}
			...
			if ($nips > $config{LF_NETBLOCK_COUNT}) {
				...
				if ($status) {
					...
					&syscommand(__LINE__,"/usr/sbin/qhtlfirewall","-d",$ipblock,"qhtlwaterfall: $message");
					logfile("$message - *Blocked in qhtlfirewall* [$active]");
					if ($config{CLUSTER_BLOCK} and $config{CLUSTER_SENDTO} and !$cluster) {&qhtlwaterfallclient(1,$message,$ipblock,"","inout","0")}
				...
				if ($config{LF_NETBLOCK_ALERT}) {
					$0 = "qhtlwaterfall - (child) sending alert email for $ipblock";
				...
				if ($status) {
					...
					&syscommand(__LINE__,"/usr/sbin/qhtlfirewall","-d",$ip,"qhtlwaterfall: $message");
					logfile("$message - *Blocked in qhtlfirewall* [$active]");
					if ($config{CLUSTER_BLOCK} and $config{CLUSTER_SENDTO} and !$cluster) {&qhtlwaterfallclient(1,$message,$ip,"","inout","0")}
	...
	if (!$blocked and !$return) {
		if ($perm) {
				...
				foreach my $dport (split(/\,/,$port)) {
					...
					if ($status) {
						...
						if (&denycheck("tcp|in|d=$dport|s=$ip",undef,$perm)) {
							...
							&syscommand(__LINE__,"/usr/sbin/qhtlfirewall","-d","$proto|in|d=$dport|s=$ip","qhtlwaterfall: $message");
							logfile("$message - *Blocked in qhtlfirewall* port=$dport [$active]");
							$blocked = 1;
						}
					}
				}
			...
			if ($blocked) {
				if ($config{CLUSTER_BLOCK} and $config{CLUSTER_SENDTO} and !$cluster) {&qhtlwaterfallclient(1,$message,$ip,$port,$inout,"0")}
			...
			if (&denycheck($ip,$port,$perm)) {
				...
				print $TEMPBAN time."|$ip|$port|$inout|$timeout|qhtlwaterfall - $message\n";
				close ($TEMPBAN);

				if ($message) {logfile("$message - *Blocked in qhtlfirewall* for $timeout secs [$active]")}
				if ($config{CLUSTER_BLOCK} and $config{CLUSTER_SENDTO} and !$cluster) {&qhtlwaterfallclient($perm,$message,$ip,$port,$inout,$timeout)}
				if ($config{BLOCK_REPORT}) {&block_report(@report)}
				if ($config{ST_ENABLE}) {&stats_report(@report)}
			}
		}
...
sub ipunblock {
	if (! -z "/var/lib/qhtlfirewall/qhtlfirewall.tempban") {
		...
		$forks{$childpid} = 1;
		unless ($childpid) {
			$0 = "qhtlwaterfall - processing temporary bans";
			my $timer = time;
			...
			unless (flock ($TEMPBAN, LOCK_EX | LOCK_NB)) {
				if ($config{DEBUG} >= 3) {logfile("debug: Unable to lock qhtlfirewall.tempban in ipunblock")}
			} else {
				my @data = <$TEMPBAN>;
				chomp @data;
	...
	if (! -z "/var/lib/qhtlfirewall/qhtlfirewall.tempallow") {
		...
		$forks{$childpid} = 1;
		unless ($childpid) {
			$0 = "qhtlwaterfall - processing temporary allows";
			my $timer = time;
			if ($config{DEBUG} >= 3) {$timer = &timer("start","ipunblock",$timer)}
			...
			unless (flock ($TEMPALLOW, LOCK_EX | LOCK_NB)) {
				if ($config{DEBUG} >= 3) {logfile("debug: Unable to lock qhtlfirewall.tempallow in ipunblock")}
			} else {
				my @data = <$TEMPALLOW>;
				chomp @data;
...
sub block_report {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) Block Report...";
...
sub stats_report {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) Stats Report...";
		...
		if (-e "/var/lib/qhtlfirewall/stats/qhtlwaterfallstats") {
			...
			sysopen (my $STATS,"/var/lib/qhtlfirewall/stats/qhtlwaterfallstats", O_RDWR | O_CREAT);
			flock ($STATS, LOCK_EX);
			seek ($STATS, 0, 0);
			truncate ($STATS, 0);
...
sub messengerrecaptcha {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - reCAPTCHA qhtlfirewall...";
...
sub messengerstop {
	...
	if ($version == 1) {
		...
		if (-e "/etc/apache2/conf.d/qhtlfirewall.messenger.conf") {
			unlink("/etc/apache2/conf.d/qhtlfirewall.messenger.conf");
			system("/scripts/restartsrv_httpd");
			logfile("*MESSENGERV2* Removed /etc/apache2/conf.d/qhtlfirewall.messenger.conf");
		}
	}
	elsif ($version == 3) {
		if (-d $config{MESSENGERV3LOCATION}) {
			if (-e $config{MESSENGERV3LOCATION}."/qhtlfirewall.messenger.conf") {
				unlink($config{MESSENGERV3LOCATION}."/qhtlfirewall.messenger.conf");
				system($config{MESSENGERV3RESTART});
				logfile("*MESSENGERV3* Removed ".$config{MESSENGERV3LOCATION}."/qhtlfirewall.messenger.conf");
			}
		}
...
sub messenger {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - messenger qhtlfirewall...";
		$SIG{INT} = \&childcleanup;
		$SIG{TERM} = \&childcleanup;
		$SIG{HUP} = \&childcleanup;
		$SIG{__DIE__} = sub {&childcleanup(@_);};
		...
		while (1) {
			...
			if ($pid == 0) {
				eval {
					...
					if ($header{cookie} =~ /qhtlfirewallsession=(\w+)/) {$cookie = $1}

					if (($session ne "" and $cookie ne "") or defined $FORM{qhtlfirewalllogin}) {
						...
						foreach my $record (@records) {
							...
							if ($rtype eq "login" and $rip eq $peeraddress and $rsession eq $session) {
								if ((time - $rtime) > $config{UI_TIMEOUT}) {
									...
									if ($rhead eq $md5sum) {
										if ($FORM{qhtlfirewallaction} eq "qhtlfirewalllogout") {
											$valid = "login";
											$record = "";
					...
					if (defined $FORM{qhtlfirewalllogin} and $valid ne "fail") {
						if ($FORM{qhtlfirewalllogin} eq $config{UI_USER} and $FORM{qhtlfirewallpassword} eq $config{UI_PASS}) {
							$valid = "yes";
						} else {
							$valid = "fail";
						}
					...
					if ($valid eq "fail") {
						...
						if ($fails{$peeraddress} > $config{UI_RETRY}) {
							if ($config{UI_BAN}) {
								sysopen (my $SESSIONBAN,"/etc/qhtlfirewall/ui/ui.ban", O_WRONLY | O_APPEND | O_CREAT) or &childcleanup(__LINE__,"UI: unable to open qhtlfirewall.session: $!");
							...
							my $time = time;
							sysopen (my $SESSION,"/var/lib/qhtlfirewall/ui/ui.session", O_WRONLY | O_APPEND | O_CREAT) or &childcleanup(__LINE__,"UI: unable to open qhtlfirewall.session: $!");
							flock ($SESSION, LOCK_EX);
					...
					if ($valid eq "yes") {
						...
						print "Set-Cookie: qhtlfirewallsession=$cookie; secure\r\n";
						print "\r\n";
					...
					if ($valid eq "login") {
						...
						print "<tr bgcolor='#F4F4EA'><td>Username:</td><td><input id='user' name='qhtlfirewalllogin' type='text' size='15'></td></tr>\n";
						print "<tr bgcolor='#F4F4EA'><td>Password:</td><td><input name='qhtlfirewallpassword' type='password' size='15'></td></tr>\n";
					...
					if ($valid eq "session") {
						...
						if ($file eq "/") {
							print "HTTP/1.0 200 OK\r\n";
							if ($application eq "qhtlfirewall") {
...
# end ui_413
###############################################################################
# start qhtlwaterfallserver
sub qhtlwaterfallserver {
	$SIG{CHLD} = 'IGNORE';
	unless (defined ($childpid = fork)) {
		&cleanup(__LINE__,"*Error* cannot fork: $!");
	} 
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall Cluster Server";
		$SIG{INT} = \&childcleanup;
		$SIG{TERM} = \&childcleanup;
		$SIG{HUP} = \&childcleanup;
		$SIG{__DIE__} = sub {&childcleanup(@_);};
						...
						if ($cmembers{$peeraddress}) {
							...
							if ($command eq "D") {
								...
								if ($config{CLUSTER_MASTER} and ($config{CLUSTER_MASTER} eq $peeraddress)) {
									...
									logfile("*Cluster* member $pip said restart qhtlfirewall and qhtlwaterfall, however it is not the CLUSTER_MASTER");
								}
							}
							else {
...
# end qhtlwaterfallserver
###############################################################################
# start qhtlwaterfallclient
sub qhtlwaterfallclient {
	my $perm = shift;
	my $message = shift;
	my $ip = shift;
	my $port = shift;
	...
	unless ($childpid) {
		my $timer = time;
		if ($config{DEBUG} >= 3) {$timer = &timer("start","qhtlwaterfallclient",$timer)}
		$0 = "qhtlwaterfall - Cluster client";
		...
		if ($config{DEBUG} >= 3) {$timer = &timer("stop","qhtlwaterfallclient",$timer)}
		$0 = "qhtlwaterfall - (child) closing";
		exit;
	}
	return;
}
# end qhtlwaterfallclient
###############################################################################
# start updateconfig
sub updateconfig {
	my $chname = shift;
...
sub stats {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) Statistics...";
...
sub systemstats {
	...
	unless ($childpid) {
		...
		$0 = "qhtlwaterfall - (child) System Statistics...";
		...
		if ($config{ST_MYSQL}) {
			...
			if ($@) {
				...
				close ($TEMPCONF);
				logfile("STATS: DBI Perl Module missing - ST_MYSQL has been temporarily disabled. You should disable ST_MYSQL and restart qhtlwaterfall if you do not use this feature");
			} else {
				eval {
					local $SIG{__DIE__} = undef;
					local $SIG{'ALRM'} = sub {die};
					...
					if ($status) {
						logfile("STATS: Unable to connect to MySQL: [$DBI::errstr] - You should disable ST_MYSQL and restart qhtlwaterfall if you do not use this feature");
					} else {
	...
	foreach my $line (@allow) {
		...
		if ($ipd eq $ipmatch) {
			...
			if (checkip(\$cidrhit)) {
				...
				if ($@) {logfile("Invalid CIDR in qhtlfirewall.allow: $cidrhit")}
