...
sub messenger {
	...
	$0 = "qhtlwaterfall $type messenger";
	...
	if ($oldtype eq "HTTPS") {
		...
		logfile("qhtlwaterfall $oldtype messenger using $vmrss kB of RSS memory at startup, adding up to $config{MESSENGER_CHILDREN} children = ".(($config{MESSENGER_CHILDREN} + 1) * $vmrss)." kB");
		logfile("qhtlwaterfall $oldtype messenger using $vmsize kB of VIRT memory at startup, adding up to $config{MESSENGER_CHILDREN} children = ".(($config{MESSENGER_CHILDREN} + 1) * $vmsize)." kB");
	}
			...
			while (1) {
				while (my $client = $server->accept()) {
					while (scalar (keys %children) >= $chldallow) {
						...
						$0 = "qhtlwaterfall $oldtype messenger (busy)";
					}
					$0 = "qhtlwaterfall $oldtype messenger";
					...
					if ($pid == 0) {
						eval {
							...
							$0 = "qhtlwaterfall $oldtype messenger client";
							...
							if (($type eq "HTML") and ($firstline =~ /^GET \/unblk\?g-recaptcha-response=(\S+)/i)) {
								...
								eval {
									...
									my $urlget = QhtLink::URLGet->new(2, "", $config{URLPROXY});
...
sub messengerv2 {
	...
	if ($ok) {
		...
		logfile("MESSENGERV2: Started Apache MESSENGERV2 service using /etc/apache2/conf.d/qhtlfirewall.messenger.conf");
	} else {
		logfile("*MESSENGERV2*: Unable to generate a valid Apache configuration, see /var/lib/qhtlfirewall/apachectl.error");
		if (-e "/etc/apache2/conf.d/qhtlfirewall.messenger.conf") {unlink("/etc/apache2/conf.d/qhtlfirewall.messenger.conf")}
		system("/scripts/restartsrv_httpd");
...
EOF
			...
			if ($line =~ /[MAPS]/) {
				...
				foreach my $map (@ssldomainkeys) {
					if (-e $ssldomains{$map}{cert}) {
						$mapping .= "map qhtlfirewallssl.${map} ${map}\n\t";
					}
				}
	...
	if (-d $config{MESSENGERV3LOCATION}) {
		...
		$location = $config{MESSENGERV3LOCATION}."/qhtlfirewall.messenger.conf";
	}
...
sub conftree {
	...
	foreach my $file (glob($fileglob)) {
		if ($file =~ /qhtlfirewall\.messenger\.conf$/) {next}
		if ($file =~ /\/var\/lib\/qhtlfirewall\/qhtlfirewall.conf$/) {next}
		if (-e $file) {
