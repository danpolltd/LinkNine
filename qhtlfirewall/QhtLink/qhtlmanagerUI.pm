...
<html lang='en'>
<head>
    <title>QhtLink File Manager</title>
    <meta charset='utf-8'>
...
EOF
    ...
    unless ($FORM{do} eq "console") {
        ...
        if ($config{UI_QHTLWATCHER} or $config{UI_QHTLMANAGER}) {
            ...
            if ($config{UI_QHTLMANAGER}) {print "<option selected>qhtlmanager</option>"}
            print "<", "/select> <input class='btn btn-default' type='submit' value='Switch'></form>\n";
        }
        print " <a class='btn btn-default' href='$script?qhtlfirewallaction=qhtlfirewalllogout'>Logout</a>\n";
        print "</div>\n";
        print <<EOF;
<div class='panel panel-default panel-body'>
<h4><span class="glyphicon glyphicon-folder-open icon-qhtlfirewall"></span> QhtLink File Manager</h4>
</div>
EOF
    }
    ...
    unless ($FORM{do} eq "console") {
        print "<p>&copy;2025, <a href='http://www.qhtlf.danpol.com'>Danpol Limited</a> (Daniel Nowakowski)</p>\n";
    }
    print <<EOF;
</div>
<script>
