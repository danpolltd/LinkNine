...
sub startoutput {
	if ($config{THIS_UI} and !$config{GENERIC}) {
		$output .= "<p align='center'><strong>Note: Internal WHM links will not work within the qhtlfirewall Integrated UI</strong></p>\n";
	}
...
sub firewallcheck {
	...
	&addline($status,"qhtlfirewall enabled check","qhtlfirewall is currently disabled and should be enabled otherwise it is not functioning");
	...
	if (-x $config{IPTABLES}) {
		...
		&addline($status,"qhtlfirewall running check","iptables is not configured. You need to start qhtlfirewall");
	}

	$status = 0;
	if ($config{TESTING}) {$status = 1}
	&addline($status,"TESTING mode check","qhtlfirewall is in TESTING mode. If the firewall is working set TESTING to \"0\" in the Firewall Configuration otherwise it will continue to be stopped");
	...
	&addline($status,"AUTO_UPDATES option check","To keep qhtlfirewall up to date and secure you should enable AUTO_UPDATES.");

	$status = 0;
	unless ($config{LF_DAEMON}) {$status = 1}
	&addline($status,"qhtlwaterfall enabled check","qhtlwaterfall is disabled in the qhtlfirewall configuration which limits the affectiveness of this application");
	...
	&addline($status,"FASTSTART option check","This option can dramatically improve the startup time of qhtlfirewall and the rule loading speed of qhtlwaterfall");

	$status = 0;
	if ($config{URLGET} == 1) {$status = 1}
	&addline($status,"URLGET option check","This option determines which perl module is used to upgrade qhtlfirewall. It is recommended to set this to use LWP rather than HTTP::Tiny so that upgrades are performed over an SSL connection");

	$status = 0;
	if ($config{PT_USERKILL} == 1) {$status = 1}
	&addline($status,"PT_USERKILL option check","This option should not normally be enabled as it can easily lead to legitimate processes being terminated, use qhtlfirewall.pignore instead");
...
#			&addline($status,"Check for DNS recursion restrictions","You have a local DNS server running but do not appear to have any recursion restrictions set. This is a security and performance risk and you should look at restricting recursive lookups to the local IP addresses only");
	...
	if (-e "/usr/local/cpanel/version" or $config{DIRECTADMIN}) {
		...
		&addline($status,"Security scanning check","Consider implementing upload and account scanning (e.g. qhtlwatcher) to reduce the risk of exploits.");
		$status = 0;
		unless (-e "/etc/qhtlmoderator/qhtlmoderator.pl") {
			$status = 1;
		}
		&addline($status,"Outgoing mail moderation","Consider monitoring and throttling outgoing email to mitigate spam abuse.");
	}
	...
	unless ($config{IPV6}) {
		...
		&addline($status,"Check for IPv6","IPv6 appears to be enabled [<b>$ipv6</b>]. If ip6tables is installed, you should enable the qhtlfirewall IPv6 firewall (IPV6 in qhtlfirewall.conf)");
	}
...
sub whmcheck {
	...
	unless ($cpconf->{nativessl} eq undef) {
		...
		&addline($status,"Check whether native cPanel SSL is enabled","You should enable this option so that qhtlwaterfall tracks SSL cpanel login attempts <i>WHM > <a href='$cpurl/scripts2/tweaksettings' target='_blank'>Tweak Settings</a> > Use native SSL support if possible, negating need for Stunnel</i>");
	}
	...
	if (-e "/var/cpanel/conf/proftpd/main" and ($cpconf->{ftpserver} eq "proftpd") and !(-e "/etc/ftpddisable")) {
		...
		if ($config{VPS}) {
			...
			&addline($status,"Check VPS FTP PASV hole","Since the Virtuozzo VPS iptables ip_conntrack_ftp kernel module is currently broken you have to open a PASV port hole in iptables for incoming FTP connections to work correctly. See the qhtlfirewall readme.txt under 'A note about FTP Connection Issues' on how to do this");
		}
	}
...
#		&addline($status,"Check security updates","You have security updating disabled, this can pose a security and stability risk. <i>WHM > <a href='$cpurl/scripts2/updateconf' target='_blank'>Update Config</a> >Operating System Package Updates > Automatic</i>");
	...
	&addline($status,"Check SMTP Restrictions","This option in WHM will not function when running qhtlfirewall. You should disable WHM > Security Center > <a href='$cpurl/scripts2/smtpmailgidonly' target='_blank'>SMTP Restrictions</a> and use the qhtlfirewall configuration option SMTP_BLOCK instead");
...
sub apachecheck {
	...
	&addline($status,"Check apache for mod_cloudflare","This module logs the real users IP address to Apache. If this is reported to qhtlwaterfall via ModSecurity, qhtlwatcher or some other vector through Apache it will lead to that IP being blocked, but because the IP is coming through the CloudFlare service the IP will <b>not</b> be blocked as so far as iptables is concerned the originating IP address is CloudFlare itself and the abuse will continue. To block these IP's in the CloudFlare Firewall look at using CF_ENABLE in qhtlfirewall.conf");
...
sub sshtelnetcheck {
	...
	if (-e "/etc/ssh/sshd_config") {
		...
		&addline($status,"Check SSH UseDNS","You should disable UseDNS by editing /etc/ssh/sshd_config and setting:<br><b>UseDNS no</b><br>Otherwise, qhtlwaterfall will be unable to track SSHD login failures successfully as the log files will not report IP addresses");
