	...
	if ($action eq "remove") {
		...
		foreach my $account (sort keys %authlist) {
			...
			if ($action eq "deny") {
				...
				print "qhtlfirewall - $status ($user)\n";
			}
			elsif ($action eq "add") {
				my $id;
				my $status;
...
sub block {
	...
	$block->{notes} = "qhtlfirewall $config{CF_BLOCK}";
...
sub whitelist {
	...
	$whitelist->{notes} = "qhtlfirewall whitelist";
...
sub challenge {
	...
	$challenge->{notes} = "qhtlfirewall challenge";
