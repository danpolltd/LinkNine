...
# Initialize slurp helpers similar to upstream
$slurpreg = QhtLink::Slurp->slurpreg;
$cleanreg = QhtLink::Slurp->cleanreg;

# Populate cPanel-related config if present (parity with upstream)
...
# Detect Apache version (if available) to set LF_APACHE_ERRPORT safely when auto (0)
...
QhtLink::Logger::logfile("LF_APACHE_ERRPORT: Set to [$config{LF_APACHE_ERRPORT}]");
...
#qhtlwatcher Apache
	if (($config{LF_QHTLWATCHER}) and ($globlogs{MODSEC_LOG}{$lgfile}) and ($line =~ /^\[\S+\s+\S+\s+\S+\s+\S+\s+\S+\] \[(\S*:)?error\] (\[pid \d+(:tid \d+)?\] )?\[(client|remote) (\S+)\]( \[client \S+\])? (\w+: )?ModSecurity:(( \[[^]]+\])*)? Access denied with code \d\d\d \(phase 2\)\. File \"[^\"]*\" rejected by the approver script \"\/qhtlwatcher\/qhtlwatcher\.sh\"/)) {
        my $ip = $5;
		my $acc = "";
		my $domain = "";
...
sub syslogcheckline {
	...
	if ($line =~ /^(\S+|\S+\s+\d+\s+\S+) \S+ qhtlwaterfall\[\d+\]: SYSLOG check \[(\S+)\]\s*$/) {
