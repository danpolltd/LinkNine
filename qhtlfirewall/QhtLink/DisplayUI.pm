...
QHTL_PLUS_BTN_CSS
			...
			my $escaped_full_click = $full_html; $escaped_full_click =~ s/\\/\\\\/g; $escaped_full_click =~ s/'/\\'/g; $escaped_full_click =~ s/\r?\n/\n/g;
			print "<script>(function(){try{var c=document.getElementById('".$container_id."');if(!c)return;var radios=c.querySelectorAll('.qhtl-tab-radio');var panels=c.querySelectorAll('.qhtl-panels .qhtl-tab-panel');var labels=c.querySelectorAll('.qhtl-tab-list label');function show(i){for(var k=0;k<panels.length;k++){panels[k].style.display=(k===i?'block':'none');}}function current(){for(var i=0;i<radios.length;i++){ if(radios[i].checked) return i; } return 0;}function isMeaningful(el){try{var t=(el.textContent||'');t=t.replace(/\u00a0|\s+/g,'');t=t.replace(/^\++$/,'');return /[A-Za-z0-9]{2,}/.test(t);}catch(_){return false;}}function maybeFill(i){ try{ if(i>0 && panels[i] && !isMeaningful(panels[i])){ panels[i].innerHTML='".$escaped_full_click."'; } }catch(__){} } show(current()); c.addEventListener('change', function(ev){ try{ var t=ev.target; if(!t || !t.matches || !t.matches('.qhtl-tab-radio')) return; var idx=current(); maybeFill(idx); show(idx); }catch(__){} }, true); for(var i=0;i<labels.length;i++){ labels[i].addEventListener('click', function(ev){ try{ var f=this.getAttribute('for'); if(!f) return; var r=document.getElementById(f); if(r && r.type==='radio'){ r.checked=true; var idx=Array.prototype.indexOf.call(radios, r); if(idx>=0){ maybeFill(idx); show(idx); } } }catch(__){} }); } }catch(_){}})();</script>\n";
