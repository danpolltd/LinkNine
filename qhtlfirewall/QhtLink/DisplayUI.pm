...
QHTL_TEMP_MODAL_JS_B
	...
	print "  root.addEventListener('click', function(ev){ var tgt=ev.target; var btn=tgt && tgt.closest ? tgt.closest('button, input[type=submit]') : null; if(btn && (String(btn.type||'').toLowerCase()==='submit')){ __qhtl_lastSubmitter=btn; } var a=tgt && tgt.closest ? tgt.closest('a') : null; if(!a) return; var href=a.getAttribute('href')||''; if(!href || href==='javascript:void(0)') return; if(!sameOrigin(href) || !isQhtlAction(href, null)) return; ev.preventDefault(); var u = href + (href.indexOf('?')>-1?'&':'?') + 'ajax=1'; loadInto(u, 'GET'); }, true);\n";
		...
		print "    box-shadow:0 0 0 10px rgba(255,215,0,0.45), 0 0 8px 2px rgba(255,230,128,0.55), inset 0 0 6px rgba(0,0,0,0.25);\n";
		print "    transition:transform .12s ease, box-shadow .12s ease;\n";
		print "  }\n";
		print "  #moreplus .qhtl-hex-btn:before{content:'';position:absolute;inset:-10px;pointer-events:none;\n";
		print "    /* bright gold halo */ background:conic-gradient(from 0deg, rgba(255,215,0,0.00) 0 10%, rgba(255,249,196,0.85) 10% 12%, rgba(255,215,0,0.00) 12% 22%, rgba(255,236,139,0.9) 22% 24%, rgba(255,215,0,0.00) 24% 34%, rgba(255,245,157,0.85) 34% 36%, rgba(255,215,0,0.00) 36% 46%, rgba(255,233,120,0.9) 46% 48%, rgba(255,215,0,0.00) 48% 58%, rgba(255,245,157,0.85) 58% 60%, rgba(255,215,0,0.00) 60% 70%, rgba(255,236,139,0.9) 70% 72%, rgba(255,215,0,0.00) 72% 82%, rgba(255,249,196,0.85) 82% 84%, rgba(255,215,0,0.00) 84% 100%);\n";
		print "    -webkit-mask: radial-gradient(circle, transparent 48%, black 49.5%, black 100%); mask: radial-gradient(circle, transparent 48%, black 49.5%, black 100%); filter: blur(.2px);\n";
		print "    animation:qhtlHaloSpin 4.5s linear infinite;\n";
		print "    clip-path:polygon(22% 3%,78% 3%,100% 50%,78% 97%,22% 97%,0% 50%); /* align ring to hex */\n";
		print "  }\n";
		print "  #moreplus .qhtl-hex-btn:hover{transform:translateY(-2px); box-shadow:0 0 0 10px rgba(255,225,100,0.6), 0 0 14px 3px rgba(255,240,160,0.9), inset 0 0 8px rgba(0,0,0,0.28);}\n";
		print "  #moreplus .qhtl-hex-btn:active{transform:translateY(0); box-shadow:0 0 0 10px rgba(255,215,0,0.5), 0 0 6px 2px rgba(255,220,120,0.75), inset 0 0 10px rgba(0,0,0,0.35);}\n";
