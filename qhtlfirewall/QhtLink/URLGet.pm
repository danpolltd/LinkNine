...
sub binget {
	...
	if ($option == 3) {
		return (1, "Unable to download (CURL/WGET also not present, see qhtlfirewall.conf)");
	} else {
		return (1, "Unable to download (CURL/WGET also not present, see qhtlfirewall.conf): ".$errormsg);
	}
}
# end binget
###############################################################################
