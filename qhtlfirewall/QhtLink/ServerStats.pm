...
sub graphs {
	...
	if (@stats > 1) {
		...
		if ($type eq "cpu") {
			...
			$img = $imghddir."qhtlwaterfall_systemhour.gif";
			open (my $OUT, ">", "$img");
			flock ($OUT, LOCK_EX);
			binmode ($OUT);
		...
		if ($type eq "cpu") {
			...
			$hour_graph->set_legend( qw(Idle IOWAIT));
			$hour_graph->plot(\@data);
			$img = $imghddir."qhtlwaterfall_systemday.gif";
			open (my $OUT, ">", "$img");
			flock ($OUT, LOCK_EX);
			binmode ($OUT);
			print $OUT $hour_graph->gd->gif();
		...
		if ($type eq "cpu") {
			...
			$hour_graph->set_legend( qw(Idle IOWAIT));
			$hour_graph->plot(\@data);
			$img = $imghddir."qhtlwaterfall_systemweek.gif";
			open (my $OUT, ">", "$img");
			flock ($OUT, LOCK_EX);
			binmode ($OUT);
			print $OUT $hour_graph->gd->gif();
		...
		if ($type eq "cpu") {
			...
			$hour_graph->set_legend( qw(Idle IOWAIT));
			$hour_graph->plot(\@data);
			$img = $imghddir."qhtlwaterfall_systemmonth.gif";
			open (my $OUT, ">", "$img");
			flock ($OUT, LOCK_EX);
			binmode ($OUT);
			print $OUT $hour_graph->gd->gif();
...
sub charts {
	...
	if (@stats) {
		...
		# Blocks by qhtlwaterfall in the last 24 hours
		my $cnt = $hour + 1;
		if ($cnt > 23) {$cnt = 0}
		my (@h,@p,@t,@hp,@cp);
		my %triggers;
		...
		$hour_graph->set(
			...
			tick_length => 0,
			x_ticks => 0, 
			title => 'Blocks by qhtlwaterfall in the last 24 hours',
			cumulate => 1,
			borderclrs => $hour_graph->{dclrs},
			bar_spacing => 4,
			shadow_depth => 1,
		...
		$hour_graph->plot(\@data);
		$img = $imghddir."qhtlwaterfall_hour.gif";
		open (my $OUT, ">", "$img");
		flock ($OUT, LOCK_EX);
		binmode ($OUT);
		print $OUT $hour_graph->gd->gif();
		...
		label => 'Trigger in qhtlfirewall.conf',
		axislabelclr => 'black',
		pie_height => 36,
		l_margin => 15,
		r_margin => 15,
		...
		$img = $imghddir."qhtlwaterfall_pie_hour.gif";
		open (my $OUT2, ">", "$img");
		flock ($OUT2, LOCK_EX);
		binmode ($OUT2);
		...
		# Blocks by qhtlwaterfall in the last 30 Days
		my $maxdays = 30;
		...
		$day_graph->set(
			...
			tick_length => 0,
			x_ticks => 0, 
			title => "Blocks by qhtlwaterfall in the last $maxdays Days",
			cumulate => 1,
		...
		$img = $imghddir."qhtlwaterfall_month.gif";
		open (my $OUT3, ">", "$img");
		flock ($OUT3, LOCK_EX);
		binmode ($OUT3);
		...
		$img = $imghddir."qhtlwaterfall_pie_day.gif";
		open (my $OUT4, ">", "$img");
		flock ($OUT4, LOCK_EX);
		binmode ($OUT4);
		...
		# Blocks by qhtlwaterfall in the last 12 months
		$cnt = $mon + 2;
		if ($cnt > 12) {$cnt = 1}
		my (@hy,@py,@ty,@yhp,@ycp);
		my %ytriggers;
		...
		$year_graph->set(
			...
			tick_length => 0,
			x_ticks => 0, 
			title => 'Blocks by qhtlwaterfall in the last 12 months',
			cumulate => 1,
		...
		$img = $imghddir."qhtlwaterfall_year.gif";
		open (my $OUT5, ">", "$img");
		flock ($OUT5, LOCK_EX);
		binmode ($OUT5);
		...
		$img = $imghddir."qhtlwaterfall_pie_year.gif";
		open (my $OUT6, ">", "$img");
		flock ($OUT6, LOCK_EX);
		binmode ($OUT6);
		...
		if ($cc_lookups) {
			# Total Top 30 Country Code blocks by qhtlwaterfall
			my (@ccy,@ccx);
			my %ccs;
			my $cntcc;
			...
			$cc_graph->set(
				...
				tick_length => 0,
				x_ticks => 0, 
				title => 'Total Top 30 Country Code blocks by qhtlwaterfall',
				cumulate => 1,
			...
			$img = $imghddir."qhtlwaterfall_cc.gif";
			open (my $OUT, ">", "$img");
			flock ($OUT, LOCK_EX);
			binmode ($OUT);
...
sub charts_html {
	...
	$html .= "<table class='table table-bordered'>\n";
	$html .= "<tr><td>\n";
	$html .= "<p align='center'><img class='img-responsive' src='".$imgdir."qhtlwaterfall_hour.gif?text=".time."'></p><p>&nbsp;</p>\n";
	$html .= "<p align='center'><img class='img-responsive' src='".$imgdir."qhtlwaterfall_pie_hour.gif?text=".time."'></p>\n";
	$html .= "</td></tr><tr><td>\n";
	$html .= "<p align='center'><img class='img-responsive' src='".$imgdir."qhtlwaterfall_month.gif?text=".time."'></p><p>&nbsp;</p>\n";
	$html .= "<p align='center'><img class='img-responsive' src='".$imgdir."qhtlwaterfall_pie_day.gif?text=".time."'></p>\n";
	if ($cc_lookups) {
		$html .= "</td></tr><tr><td>\n";
		$html .= "<p align='center'><img class='img-responsive' src='".$imgdir."qhtlwaterfall_year.gif?text=".time."'></p><p>&nbsp;</p>\n";
		$html .= "<p align='center'><img class='img-responsive' src='".$imgdir."qhtlwaterfall_pie_year.gif?text=".time."'></p>\n";
		$html .= "</td></tr>\n<tr><td>\n";
		$html .= "<p align='center'><img class='img-responsive' src='".$imgdir."qhtlwaterfall_cc.gif?text=".time."'></p>\n";
	} else {
		$html .= "</td></tr>\n<tr><td>\n";
		$html .= "<p align='center'><img class='img-responsive' src='".$imgdir."qhtlwaterfall_year.gif?text=".time."'></p><p>&nbsp;</p>\n";
		$html .= "<p align='center'><img class='img-responsive' src='".$imgdir."qhtlwaterfall_pie_year.gif?text=".time."'></p>\n";
