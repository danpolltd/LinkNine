...
if ($config{CC_LOOKUPS} == 4) {
	$urlget = QhtLink::URLGet->new($config{URLGET}, "", $config{URLPROXY});
...
sub iplookup {
	...
	if ($config{LF_LOOKUPS} and !$cconly) {
		...
		if ($cachehit) {
			...
			sysopen (DNS, "/var/lib/qhtlfirewall/qhtlfirewall.dnscache", O_WRONLY | O_APPEND | O_CREAT);
			flock (DNS, LOCK_EX);
