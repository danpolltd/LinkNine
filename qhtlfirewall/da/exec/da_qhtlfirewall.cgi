...
our %session;
our @sessiondata;
unless (-e "/var/lib/qhtlfirewall/qhtlfirewall.da.skip") {
	if ($ENV{SESSION_ID} =~ /^\w+$/) {
...
$script = "/CMD_PLUGINS_ADMIN/qhtlfirewall/index.raw";
$script_da = "/CMD_PLUGINS_ADMIN/qhtlfirewall/index.raw";
$images = "/CMD_PLUGINS_ADMIN/qhtlfirewall/images";
...
sub loginfail {
	my $message = shift;
	my $file = "/var/lib/qhtlfirewall/da".time.".error";
	print $message."<p>Information saved to [$file]\n";
	sysopen (my $FILE, $file, O_WRONLY | O_CREAT | O_TRUNC);
	flock ($FILE, LOCK_EX);
	print $FILE "To disable DirectAdmin session checks, create a touch file called /var/lib/qhtlfirewall/qhtlfirewall.da.skip\n\n";
	print $FILE $message."\n\n";
