...
sub main {
	...
	if (($FORM{ip} ne "") and ($FORM{ip} ne "all") and (!checkip(\$FORM{ip}))) {
		...
		ConfigServer::Service::statusqhtlwaterfall();
		print "</pre>\n<p>...<b>Done</b>.</div>\n";
		&printreturn;
	}
...
EOF
		...
		print "<table class='table table-bordered table-striped'>\n";
		print "<thead><tr><th colspan='2'>qhtlfirewall - ConfigServer Firewall</th></tr></thead>";
		...
		if ($config{CLUSTER_SENDTO}) {
			...
			print "<table class='table table-bordered table-striped'>\n";
			print "<thead><tr><th colspan='2'>qhtlfirewall - ConfigServer qhtlwaterfall Cluster</th></tr></thead>";
...
sub chart {
	...
	if (-e "/usr/local/cpanel/version") {
		...
		$imgdir = "/configserver/qhtlfirewall/";
		$imghddir = "/usr/local/interworx/html/configserver/qhtlfirewall/";
		umask(0133);
	}
	elsif (-e "/usr/local/CyberCP/") {
		$imgdir = "/static/configserverqhtlfirewall/";
		$imghddir = "/usr/local/CyberCP/public/static/configserverqhtlfirewall/";
		umask(0133);
	}
	if ($config{THIS_UI}) {
		$imgdir = "$images/";
...
sub systemstats {
	...
	if (-e "/usr/local/cpanel/version") {
		if (-e "/usr/local/cpanel/bin/register_appconfig") {
			$imgdir = "qhtlfirewall/";
			$imghddir = "cgi/configserver/qhtlfirewall/";
		} else {
			$imgdir = "/";
			$imghddir = "";
		}
