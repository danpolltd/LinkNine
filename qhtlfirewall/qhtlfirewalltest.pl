...
if ($return ne "") {
	print "FAILED [FATAL Error: $return] - Required for qhtlfirewall to function\n";
	$fatal++;
} else {
	print "OK\n";
...
if ($return ne "") {
	print "FAILED [Error: $return] - Required for qhtlfirewall.redirect feature\n";
	$error++;
} else {
	print "OK\n";
...
if ($fatal) {print "\nRESULT: qhtlfirewall will not function on this server due to FATAL errors from missing modules [$fatal]\n"}
elsif ($error) {print "\nRESULT: qhtlfirewall will function on this server but some features will not work due to some missing iptables modules [$error]\n"}
else {print "\nRESULT: qhtlfirewall should function on this server\n"}
