    ...
    mountInline: function(anchor){
      try {
        ...
        try {
          ...
          inner.addEventListener('click', function(e){ e.preventDefault(); var base=(window.QHTL_SCRIPT||''); var url = base + '?action=dirwatch'; try{ window.location = url; } catch(_){ location.href = url; } });
        } catch(_){ }
        el.appendChild(node);
        return true;
