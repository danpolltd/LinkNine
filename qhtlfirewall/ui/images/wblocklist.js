(function(){
  window.WBlocklist = {
    mountInline: function(anchor){
      try {
        var el = typeof anchor==='string'?document.querySelector(anchor):anchor; if(!el) return false; ensure(); var node = make('Blocklist');
        try {
          var inner = node.querySelector('.wcircle-inner');
          inner.style.cursor = 'pointer';
          inner.addEventListener('click', function(e){ e.preventDefault(); var base=(window.QHTL_SCRIPT||''); var url = base + '?action=blocklists'; try{ window.location = url; } catch(_){ location.href = url; } });
        } catch(_){ }
        el.appendChild(node);
        return true;
