...
elsif (($input{command} eq "--disable") or ($input{command} eq "-x")) {&qhtlfirewalllock("lock");&dodisable;&qhtlfirewalllock("unlock")}
elsif (($input{command} eq "--enable") or ($input{command} eq "-e")) {&qhtlfirewalllock("lock");&doenable;&qhtlfirewalllock("unlock")}
...
elsif ($input{command} eq "--qhtlwaterfall") {&doqhtlwaterfall}
...
sub dostart {
	...
	my $qhtlfirewallpre;
	my $qhtlfirewallpost;
	if (-e "/usr/local/qhtlfirewall/bin/qhtlfirewallpre.sh") {$qhtlfirewallpre = "/usr/local/qhtlfirewall/bin/qhtlfirewallpre.sh"}
	elsif (-e "/etc/qhtlfirewall/qhtlfirewallpre.sh") {$qhtlfirewallpre = "/etc/qhtlfirewall/qhtlfirewallpre.sh"}
	if (-e "/usr/local/qhtlfirewall/bin/qhtlfirewallpost.sh") {$qhtlfirewallpost = "/usr/local/qhtlfirewall/bin/qhtlfirewallpost.sh"}
	elsif (-e "/etc/qhtlfirewall/qhtlfirewallpost.sh") {$qhtlfirewallpost = "/etc/qhtlfirewall/qhtlfirewallpost.sh"}

	if ($qhtlfirewallpre ne "") {
		chmod (0700,$qhtlfirewallpre);
		my @conf = slurp($qhtlfirewallpre);
		if ($conf[0] !~ /^\#\!/) {
			open (my $CONF, ">", $qhtlfirewallpre);
			flock ($CONF, LOCK_EX);
			print $CONF "#!/bin/bash\n";
		...
		print "Running $qhtlfirewallpre\n";
		&syscommand(__LINE__,"$path ; $qhtlfirewallpre");
	}
	...
	if ($qhtlfirewallpost ne "") {
		chmod (0700,$qhtlfirewallpost);
		my @conf = slurp($qhtlfirewallpost);

		if ($conf[0] !~ /^\#\!/) {
			open (my $CONF, ">", $qhtlfirewallpost);
			flock ($CONF, LOCK_EX);
			print $CONF "#!/bin/bash\n";
			foreach my $line (@conf) {
		        $line =~ s/$cleanreg//g;
		...
		print "Running $qhtlfirewallpost\n";
		&syscommand(__LINE__,"$path ; $qhtlfirewallpost");
	}
		...
		if ($data[0] =~ /^(\d+)\.(\d+)\.(\d+)/) {
			...
			if (($maj > 2) or (($maj > 1) and ($mid > 5) and ($min > 26))) {
				...
				if (-e "/etc/pure-ftpd.conf") {
					...
					if ($status) {$warning .= "*WARNING* Since the Virtuozzo VPS iptables ip_conntrack_ftp kernel module is currently broken you have to open a PASV port hole in iptables for incoming FTP connections to work correctly. See the qhtlfirewall readme.txt under 'A note about FTP Connection Issues' on how to do this if you have not already done so.\n"}
				}
...
sub doadd {
	...
	if ($hit) {
		print "Removing $ip from qhtlfirewall.deny...\n";
		$input{argument} = $ip;
		&dokill;
	}
...
sub dodisable {
	...
	unless ($config{GENERIC}) {
		...
		truncate ($CONF, 0);
		foreach my $line (@conf) {
			if ($line =~ /^qhtlwaterfall:/) {$line = "qhtlwaterfall:0"}
			print $CONF $line."\n";
		}
	...
	if ($config{DIRECTADMIN}) {
		...
		truncate ($CONF, 0);
		foreach my $line (@conf) {
			if ($line =~ /^qhtlwaterfall=/) {$line = "qhtlwaterfall=OFF"}
			print $CONF $line."\n";
		}
...
sub doenable {
	...
	unless ($config{GENERIC}) {
		...
		truncate ($CONF, 0);
		foreach my $line (@conf) {
			if ($line =~ /^qhtlwaterfall:/) {$line = "qhtlwaterfall:1"}
			print $CONF $line."\n";
		}
		close ($CONF) or &error(__LINE__,"Could not close /etc/conf: $!");
		open (my $OUT, ">", "/etc/chkserv.d/qhtlwaterfall");
		flock ($OUT, LOCK_EX);
	...
	if ($config{DIRECTADMIN}) {
		...
		truncate ($CONF, 0);
		foreach my $line (@conf) {
			if ($line =~ /^qhtlwaterfall=/) {$line = "qhtlwaterfall=ON"}
			print $CONF $line."\n";
		}
		close ($CONF) or &error(__LINE__,"Could not close /usr/local/directadmin/data/admin/services.status: $!");
	}
...
sub error {
	...
	flock ($OUT, LOCK_EX);
	print $OUT "Error: $error, at line $line in /usr/sbin/qhtlfirewall\n";
	close ($OUT);
	if ($config{TESTING}) {&crontab("remove")}
	exit 1;
}
	...
	foreach my $line (@allow) {
		...
		if ($ipd eq $ipmatch) {
			...
			if (checkip(\$cidrhit)) {
				...
				eval {local $SIG{__DIE__} = undef; $cidr->add($cidrhit)};
				if ($cidr->find($ipmatch)) {
					print "\nPermanent Allows (qhtlfirewall.allow): $line\n"
				}
			}
		}
	}
	...
	foreach my $line (@deny) {
		...
		if ($ipd eq $ipmatch) {
			...
			if (checkip(\$cidrhit)) {
				...
				eval {local $SIG{__DIE__} = undef; $cidr->add($cidrhit)};
				if ($cidr->find($ipmatch)) {
					print "\nPermanent Blocks (qhtlfirewall.deny): $line\n"
				}
			}
		}
	}
...
sub dologrun {
	if ($config{LOGSCANNER}) {
		...
		print "Option LOGSCANNER needs to be enabled in qhtlfirewall.conf for this feature\n";
	}
	return;
}
# end dologrun
...
sub doprofile {
	...
	if ($cmd eq "list") {
		...
		if (-e "/var/lib/qhtlfirewall/backup/$profile1") {
			...
			print "You should now restart qhtlfirewall and then qhtlwaterfall\n";
		} else {
		...
		if (-e "/usr/local/qhtlfirewall/profiles/${profile1}.conf") {
			...
			print "[$profile1] has been applied. You should now restart qhtlfirewall and then qhtlwaterfall\n";
		} else {
...
sub docloudflare {
	...
	unless ($config{CF_ENABLE}) {
		print "qhtlfirewall - CF_ENABLE must be enabled and CloudFlare access details configured to use these commands\n";
		exit 1;
	}
...
sub dographs {
	...
	if ($type eq "" and $dir eq "") {
		...
		print "\n";
		print "Usage: qhtlfirewall [graph type] [directory]\n";
		exit 1;
	}
...
sub syscommand {
	...
	if ($status) {
		...
		if ($output[0] =~ /xtables lock/) {
			$warning .= "iptables command [$command] failed due to xtables lock, enable WAITLOCK in qhtlfirewall.conf\n\n";
		}
...
sub faststart {
	...
	if (@faststart4) {
		...
		if ($results[0] =~ /^(iptables|ip6tables|xtables|Bad|Another)/) {
			...
			&error(__LINE__,"FASTSTART: ($text IPv4) [$cmd] [$results[0]]. Try restarting qhtlfirewall with FASTSTART disabled");
		}
	...
	if (@faststart4nat) {
		...
		if ($results[0] =~ /^(iptables|ip6tables|xtables|Bad|Another)/) {
			my $cmd;
			if ($results[1] =~ /^Error occurred at line: (\d+)$/) {$cmd = $faststart4[$1 - 1]}
			&error(__LINE__,"FASTSTART: ($text IPv4nat) [$cmd] [$results[0]]. Try restarting qhtlfirewall with FASTSTART disabled");
		}
		&iptableslock("unlock",1);
	}
	if (@faststart6 and $config{IPV6}) {
		...
		if ($results[0] =~ /^(iptables|ip6tables|xtables|Bad|Another)/) {
			my $cmd;
			if ($results[1] =~ /^Error occurred at line: (\d+)$/) {$cmd = $faststart4[$1 - 1]}
			&error(__LINE__,"FASTSTART: ($text IPv6) [$cmd] [$results[0]]. Try restarting qhtlfirewall with FASTSTART disabled");
		}
		&iptableslock("unlock",1);
	}
	if (@faststart6nat and $config{IPV6}) {
		...
		if ($results[0] =~ /^(iptables|ip6tables|xtables|Bad|Another)/) {
			...
			&error(__LINE__,"FASTSTART: ($text IPv6nat) [$cmd] [$results[0]]. Try restarting qhtlfirewall with FASTSTART disabled");
		}
		&iptableslock("unlock",1);
	}
	if (@faststartipset) {
		...
		chomp @results;
		if ($results[0] =~ /^ipset/) {
			print "FASTSTART: (IPSET) Error:[$results[0]]. Try restarting qhtlfirewall with FASTSTART disabled";
		}
	}
	undef @faststart4;
	undef @faststart4nat;
