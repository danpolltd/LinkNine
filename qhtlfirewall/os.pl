...
if (-e "/etc/redhat-release") {
	...
	open (my $IN, "<", "qhtlfirewall.generic.conf") or die $!;
	flock ($IN, LOCK_SH) or die $!;
	my @config = <$IN>;
	close ($IN);
	chomp @config;
	open (my $OUT, ">", "qhtlfirewall.generic.conf") or die $!;
	...
	close ($OUT);
	print STDERR "Configuration modified for SuSE settings /etc/qhtlfirewall/qhtlfirewall.conf\n";
}
	...
	close ($IN);
	chomp @config;
	open (my $GENERIC, ">", "qhtlfirewall.generic.conf") or die $!;
	...
	open (my $DIRECTADMIN, "<", "qhtlfirewall.directadmin.conf") or die $!;
	flock ($DIRECTADMIN, LOCK_SH) or die $!;
	@config = <$DIRECTADMIN>;
	close ($DIRECTADMIN);
	chomp @config;
	open (my $OUT, ">", "qhtlfirewall.directadmin.conf") or die $!;
	flock ($OUT, LOCK_EX) or die $!;
	foreach my $line (@config) {
		if ($line =~ /^FUSER/) {$line = 'FUSER = "/bin/fuser"'}
		if ($line =~ /^SYSTEMCTL /) {$line = 'SYSTEMCTL = "/bin/systemctl"'}
	...
	close ($OUT);
	print STDERR "Configuration modified for Debian/Ubuntu/Gentoo settings /etc/qhtlfirewall/qhtlfirewall.conf\n";

	open (my $IN, "<", "qhtlfirewall.cyberpanel.conf") or die $!;
	flock ($IN, LOCK_SH) or die $!;
	my @config = <$IN>;
	close ($IN);
	chomp @config;
	open (my $GENERIC, ">", "qhtlfirewall.cyberpanel.conf") or die $!;
	flock ($GENERIC, LOCK_EX) or die $!;
	foreach my $line (@config) {
		if ($line =~ /^FUSER/) {$line = 'FUSER = "/bin/fuser"'}
	...
	close ($OUT);
	print STDERR "Configuration modified for Slackware settings /etc/qhtlfirewall/qhtlfirewall.conf\n";
