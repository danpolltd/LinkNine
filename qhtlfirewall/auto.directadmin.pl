...
if (-e "/etc/qhtlfirewall/qhtlfirewall.allow") {
	...
	truncate (IN, 0);
	foreach my $line (@data) {
		if ($line =~ /^Include \/etc\/qhtlfirewall\/cpanel\.comodo\.allow/) {next}
		print IN "$line\n";
	}
	close (IN);
}
if (-e "/etc/qhtlfirewall/qhtlfirewall.ignore") {
	...
	truncate (IN, 0);
	foreach my $line (@data) {
		if ($line =~ /^Include \/etc\/qhtlfirewall\/cpanel\.comodo\.ignore/) {next}
		print IN "$line\n";
	}
	close (IN);
}
...
open (IN, "<", "qhtlfirewall.directadmin.conf") or die $!;
flock (IN, LOCK_SH) or die $!;
my @config = <IN>;
close (IN);
chomp @config;
...
foreach my $line (@config) {
	...
	if (&checkversion("14.03") and !-e "/var/lib/qhtlfirewall/auto1403" and $name eq "DIRECTADMIN_LOG_R") {
...
if (&checkversion("14.03") and !-e "/var/lib/qhtlfirewall/auto1403") {
		open (my $AUTO, ">", "/var/lib/qhtlfirewall/auto1403");
		flock ($AUTO, LOCK_EX);
		print $AUTO time;
		close ($AUTO);
}
